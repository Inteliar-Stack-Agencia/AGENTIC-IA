// Proxy server-side hacia la Edge Function get-expenses.
...
const EDGE_FUNCTION_URL = "https://pjrhfbhqdbyoljactdkj.supabase.co/functions/v1/get-expenses";
...
const PERMITIDOS = ["supplier_id", "category", "expense_type", "from", "to", "limit"];
...
export async function onRequestGet(context) {
  ...
  if (!res.ok) {
    ...
    return json({ error: `get-expenses respondió ${res.status}: ${detail}` }, res.status);
  }
