// Proxy server-side hacia la Edge Function get-company-invoices.
...
const EDGE_FUNCTION_URL = "https://pjrhfbhqdbyoljactdkj.supabase.co/functions/v1/get-company-invoices";
...
const PERMITIDOS = ["client_id", "from", "to", "status", "limit"];
...
export async function onRequestGet(context) {
  ...
  if (!res.ok) {
    ...
    return json({ error: `get-company-invoices respondió ${res.status}: ${detail}` }, res.status);
  }
